import type { Category, GraphNode } from "../types.js";
...
const RULES: Rule[] = [
  {
    category: "test",
    test: (id) => /(\.|_|-)(test|spec)\.|(^|\/)(tests?|__tests__|spec)\//i.test(id),
  },
  {
    category: "config",
    test: (id, label) =>
  ...
  },
  {
    category: "entry",
    test: (id, label) =>
      /^(main|index|app|server|cli|program)\.[\w]+$/i.test(label) &&
      /(^|\/)(src\/)?(main|index|app|server|cli|program)\.[\w]+$/i.test(id),
  },
  {
    category: "component",
    test: (id, label) =>
...
export function categorize(node: GraphNode): Category {
  for (const rule of RULES) {
    if (rule.test(node.id, node.label)) return rule.category;
  }
  return "unknown";
}
